...
void match_categories(struct json *js_rd) { // must have atleast one category (provide user option?)
	while (true) {
		...
		match_string("id");
		match(':');
		const int cat_id = std::stoi(match_number());
		js_rd->categories.append("{\"id\":" + std::to_string(cat_id));
		match(',');
		match_string("name");
		match(':');
		const std::string cat_name = get_string();
...
void add_image(const std::string date_captured, const std::string file_name, const int h_img, const int id, const int w_img) {
...
void add_annotation(const int area, const int bbox_x, const int bbox_y, const int bbox_w, const int bbox_h, const int cat_id, const int id, const int img_id) {
